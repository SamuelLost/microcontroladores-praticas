...
void TPM0_IRQHandler(void){
	//if((TPM_REG->CNT & TPM_REG->MOD) != 0){GPIO_B->PTOR = (1 << 18);}
	if((TPM_REG->CNT & TPM_REG->MOD) != 0){
		GPIO_E->PTOR = (1 << 31);
	}
...
int main(void) {
	...
	SOPT2_REG->SOPT2 = (1 << 24);//clock MCGFLLCLK é de 20,97
	...
	//TPM_REG->MOD = PM_CLOCK / (1 << (TPM_clk_PRESCALE + 1))/TPM_OVerflow_frequency
	TPM_REG->MOD = 0x10000; //65536;

	TPM_REG->CNT = 0x0000; // resetar

	//(PS)  Prescaler
	TPM_REG->SC = (0b111 << 0);//ativado o prescaler para 128
